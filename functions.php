...
if ( ! function_exists( 'helloworld_setup' ) ) :
	...
	function helloworld_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'helloworld' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'helloworld_custom_background_args',
				array(
...
function helloworld_scripts() {
	wp_enqueue_style( 'helloworld-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'helloworld-style', 'rtl', 'replace' );

	wp_enqueue_script( 'helloworld-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
